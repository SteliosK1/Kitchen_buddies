...
app.put('/api/user-recipes/:id', async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.execute(
            'UPDATE user_recipes SET title = ?, image = ?, instructions = ?, ingredients = ?, cook_time = ?, rating = ? WHERE id = ? AND user_id = ?',
            [title, imageUrl, instructions, JSON.stringify(ingredients), cookTime, rating, id, userId]
        );
